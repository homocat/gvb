...
type ArticleModel struct {
	...
	UserModel     UserModel      `gorm:"foreignKey:UserID" json:"-"`              // 文章标题
	UserID        uint           `json:"user_id"`                                 // 用户id
	Category      string         `gorm:"size:20" json:"category"`                 // 文章分类
	Source        string         `json:"source"`                                  // 文章来源
	Link          string         `json:"json"`                                    // 原文链接
	...
	Tags          ctype.Array    `gorm:"type:strings;size:64" json:"tags"`        // 文章标签
}
